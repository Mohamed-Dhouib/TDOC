"""Wavelet-based feature fusion neck copied from the official FFDN repo."""
...
try:
    ...
    raise ImportError('mmsegmentation must be installed to use DWTFPN (missing FPN)') from exc
...
class RB(nn.Module):
    def __init__(self, channels, lf):
        super().__init__()
        # lf=True uses InstanceNorm (AGFG_LL), lf=False uses BatchNorm (AGFG_HH)
...
class GF(nn.Module):
    ...
    def forward(self, lr_x, lr_y, hr_x, l_a):
        ...
        N = self.boxfilter(lr_x.new_ones((1, 1, h_lrx, w_lrx)))
...
class DWTNeck(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.DWT = DWT()
        self.AGFG_LL = AGFG(out_channels, True)   # InstanceNorm + mean_conv/std_conv
        self.AGFG_HH = AGFG(out_channels, False)  # BatchNorm (checkpoint has mean_conv but lf=False)
...
class DWTFPN(nn.Module):
    def __init__(self, **kwargs):
        ...
        if 'in_channels' not in kwargs or 'out_channels' not in kwargs:
            raise ValueError('DWTFPN expects "in_channels" and "out_channels" in kwargs')
        ...
        fpn_kwargs = dict(kwargs)
        fpn_kwargs['in_channels'] = [out_channels] * 4
        self.fpn = FPN(**fpn_kwargs)
