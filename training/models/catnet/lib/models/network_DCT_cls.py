...
class DCT_Stream_cls(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(DCT_Stream_cls, self).__init__()
        ...
        self.incre_modules, self.downsamp_modules, \
            self.final_layer = self._make_head(DC_final_stage_channels)
...
def get_cls_net(config, **kwargs):
    model = DCT_Stream_cls(config, **kwargs)
    model.init_weights()
    return model
